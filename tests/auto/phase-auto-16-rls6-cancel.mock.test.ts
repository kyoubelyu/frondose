...
const workflowMod = await import("../../src/cli/subcommands/serve/routes/workflow.js").catch(() => null);
...
function makeFakeReq(body: unknown = {}) {
  ...
  const readable = {
    ...
    on(_event: string, _cb: (...args: unknown[]) => void) { return readable; },
  };
...
function makeState(opts: { autoRunId?: string | null } = {}) {
  const abortSpy = { called: 0, abort() { this.called++; } };
  const state: AnyObj = {
  ...
  const deps: AnyObj = {
    ...
    workflow: {
      handleEndpoint(_url: string, _body: unknown) {
        return opts.controllerResponse ?? {
          status: 200,
...
function makeTurn() {
  return { resumeWorkflowTurn: async () => {} } as unknown as ReturnType<typeof import("../../src/cli/subcommands/serve/turn.js").createTurnRunner>;
}
...
describe("T-A16.RLS6 — /workflow/cancel DB-authoritative re-derive + always-abort (P-AUTO-16 RLS-6)", () => {
  ...
  // ─── G-A16.1 ───────────────────────────────────────────────────────────────
  it(
    "T-A16.RLS6.1: when getCurrentAutoRun returns a running row and state.autoRunId=null, endAutoRun IS called once and emitFrame IS called once (re-derived path)",
    async () => {
  ...
  // ─── G-A16.2 ───────────────────────────────────────────────────────────────
  it(
    "T-A16.RLS6.2: when getCurrentAutoRun returns null, abort() IS called once AND endAutoRun NOT called AND emitFrame NOT called",
    async () => {
  ...
  // ─── G-A16.10 ──────────────────────────────────────────────────────────────
  it(
    "T-A16.RLS6.10: idempotency — reaper-closed run (getCurrentAutoRun returns null) → endAutoRun NOT called, emitFrame NOT called, abort still fires",
    async () => {
  ...
  // ─── G-A16.14 ──────────────────────────────────────────────────────────────
  it(
    "T-A16.RLS6.14: non-Auto (Manual/Magical) turn with no DB run → abort fires, emitFrame NOT called, response is controller's no_workflow envelope",
    async () => {
  ...
  // ─── G-A16.15 ──────────────────────────────────────────────────────────────
  it(
    "T-A16.RLS6.15: race — getCurrentAutoRun returns a row but endAutoRun returns alreadyEnded=true → closedAutoRun=false → no early-return, falls through to controller response",
    async () => {
      ...
      assert.ok(idempotentResult.alreadyEnded === true, "endAutoRun on an already-closed row must return alreadyEnded:true");
      ...
      assert.equal(afterClose, null, "after closing the row, getCurrentAutoRun must return null (status='running' filter)");
