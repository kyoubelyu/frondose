...
describe(
  "runAuthSubcommand set — model field flows through non-interactively (G-P36.11)",
  () => {
    it(
...
describe(
  "src/cli/main.ts auth set registration uses --model-id option (G-P36.11)",
  () => {
    it(
...
describe(
  "auth.ts guidance strings all use --model-id, never --model (G-P36.12)",
  () => {
    it(
      ...
      () => {
        ...
        assert.ok(
          authTs.includes("--model-id"),
          "T-FE.3: auth.ts must use '--model-id' in guidance strings",
        );
...
describe(
  "auth list header names secrets.json path; empty-state says 'No providers configured' (G-P36.13 + NIT-1)",
  () => {
    it(
      ...
      async () => {
        ...
        try {
          const stdoutB = await captureStdout(() =>
            runAuthSubcommand("list", { authPath: authPathB }),
          );
