...
describe("autoUpdate — lock primitives", () => {
  ...
  it(
    "T-autoUpdate.Lock.2: when an existing lock file is older than LOCK_STALE_MS (45 min), acquireUpdateLock reclaims it; a fresh lock older than 44 min is NOT reclaimed (EEXIST)",
    () => {
...
describe("autoUpdate — symlink helpers", () => {
  ...
  it(
    "T-autoUpdate.Symlink.2: isDevLink returns true when realpath contains .git; false when no .git; false when symlink is broken/missing",
    () => {
      ...
      try {
        ...
        assert.strictEqual(
          isDevLink(devSymlink),
          true,
          "symlink pointing to a dir with .git should return true",
        );
        ...
        assert.strictEqual(
          isDevLink(brokenSymlink),
          false,
          "broken/dangling symlink should return false",
        );
      } finally {
        rmSync(tmp, { recursive: true, force: true });
      }
...
describe("autoUpdate — GC", () => {
  it(
    "T-autoUpdate.Gc.1: gcOldReleases(releasesDir, 2) keeps the 2 newest versions and removes older; semver-aware; no-op when count <= keepLastN; silent when dir missing",
    () => {
