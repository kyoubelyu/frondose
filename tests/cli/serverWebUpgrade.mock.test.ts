...
function makeFakeDuplex(): { socket: FakeSocket } {
  ...
  socket.end = (): FakeSocket => { socket.emit("close"); return socket; };
...
describe("T-UPG: attachWebSockets upgrade handler (G-P30.4, G-P30.5, G-P30.6, G-P30.7, G-P30.18)", () => {
  it(
    "T-UPG.1: given webToken set, an upgrade with valid Authorization: Basic header → upgrade accepted (no 401)",
    async () => {
  ...
  it(
    "T-UPG.2: given webToken set, an upgrade with no/wrong Authorization header → socket gets HTTP/1.1 401 + WWW-Authenticate, then destroyed",
    async () => {
      ...
      try {
        ...
        assert.ok(
          writes.includes("401"),
          `T-UPG.2: missing auth must produce HTTP 401; got: ${writes.slice(0, 200)}`,
        );
        assert.ok(
  ...
  it(
    "T-UPG.3: given webToken undefined, upgrade request with no Authorization header → upgrade accepted (no-auth mode)",
    async () => {
      ...
      try {
        ...
        assert.ok(
          !writes.includes("401"),
          `T-UPG.3: no-auth mode must NOT produce 401; got: ${writes.slice(0, 200)}`,
        );
      } finally {
        cleanup();
      }
  ...
  it(
    "T-UPG.4: /ws/ssh/<id> → 101 WS upgrade; /ws/vnc/<id> → 101 WS upgrade; /ws/other/<id> → socket destroyed immediately with no response",
    async () => {
      ...
      try {
        ...
        assert.equal(
          otherSocket.writtenChunks.length,
          0,
          "T-UPG.4: /ws/other must write nothing before destroy",
        );
      } finally {
        cleanup();
      }
  ...
  it(
    "T-UPG.5: given /ws/ssh/ghost where 'ghost' is not an active worker, socket is destroyed without dispatching handleSshWs",
    async () => {
  ...
  it(
    "T-UPG.6 (regression): GET /api/web/workers still returns 200 + workers array after attachWebSockets is wired up",
    async () => {
...
describe("T-UPG.attachWebSockets: direct stub behavior (G-P30.4)", () => {
  it(
    "T-UPG.attachWebSockets: attachWebSockets is exported and callable without throwing",
    () => {
