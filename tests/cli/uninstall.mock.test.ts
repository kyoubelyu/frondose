...
describe("mai uninstall — plain uninstall (G-P38.3)", () => {
  it(
    "T-UI.1: given full tmp install layout, confirm→true, no --purge: bin symlink + pkg symlink + releases dir removed; secrets.json preserved",
    async () => {
      ...
      try {
        ...
        try { lstatSync(inst.argv1); binStatBefore = true; } catch { /* */ }
        ...
        assert.ok(existsSync(join(inst.maiDir, "secrets.json")), "precondition: secrets.json must exist before uninstall");
        ...
        let binExists = false;
        try { lstatSync(inst.argv1); binExists = true; } catch { /* expected — ENOENT */ }
        ...
        assert.ok(
          ...
          "secrets.json must be preserved (--purge NOT passed)",
        );
      } finally {
        inst.cleanup();
      }
  ...
  it(
    "T-UI.1b: given confirm→true, no --purge: stdout contains daemon-warning ('mai server uninstall' / 'mai telegram off') — CONCERN-1 D-8 fix",
    async () => {
...
describe("mai uninstall — non-global install (G-P38.4)", () => {
  it(
    "T-UI.2: given argv1 is a plain (non-symlink) file, derivePackageSymlink→null: prints 'not a global install'; nothing removed",
    async () => {
      ...
      try {
        const stdout = await captureStdout(async () => {
          await runUninstallSubcommand({
            ...
            argv1: plainFile,
            homeDir: tmpDir,
            confirm: async () => { throw new Error("confirm must NOT be called for non-global install"); },
          });
        });
...
describe("mai uninstall — confirm→false aborts (G-P38.5)", () => {
  it(
    "T-UI.3: given confirm→false: prints abort message; bin symlink, pkg symlink, releases dir all still exist",
    async () => {
      ...
      try {
        ...
        let binExists = false;
        try { lstatSync(inst.argv1); binExists = true; } catch { /* */ }
...
describe("mai uninstall — --yes skips confirm (G-P38.5)", () => {
  it(
    "T-UI.4: given yes:true: confirm is NOT called; removal proceeds",
    async () => {
...
describe("mai uninstall --purge — removes ~/.mai/ (G-P38.6)", () => {
  it(
    "T-UI.5: given purge:true, both confirms→true: ~/.mai/ fully removed",
    async () => {
...
describe("mai uninstall --purge — warning text (G-P38.6)", () => {
  it(
    "T-UI.6: given purge:true, confirms→true: stdout contains 'chrome-profile' + 'mai-browser' + 'IRREVERSIBLY' (or equivalent purge-destruction warning)",
    async () => {
      // Given: full tmp layout; purge:true; both confirms return true
      ...
      try {
        ...
        assert.ok(
          stdout.includes("chrome-profile"),
          `stdout must mention 'chrome-profile'. Got: ${stdout}`,
        );
        assert.ok(
          stdout.includes("IRREVERSIBLY"),
          `stdout must mention 'IRREVERSIBLY'. Got: ${stdout}`,
        );
        assert.ok(
          stdout.includes("mai-browser"),
          `stdout must mention 'mai-browser'. Got: ${stdout}`,
        );
      } finally {
        inst.cleanup();
      }
...
describe("mai uninstall — pkg path is real directory (G-P38.9)", () => {
  it(
    "T-UI.7: given pkg symlink path is actually a real directory (not a symlink): removed via rmSync({recursive}), not left behind",
    async () => {
      ...
      try {
        ...
        assert.ok(!lstatSync(inst.pkgSymlinkPath).isSymbolicLink(), "precondition: pkg path must be real dir, not symlink");
        ...
        assert.ok(
          !existsSync(inst.pkgSymlinkPath),
          "real-dir pkg path must be removed via rmSync({recursive})",
        );
      } finally {
        inst.cleanup();
      }
...
describe("mai uninstall — no child_process import (G-P38.7)", () => {
  it(
    "T-UI.8: uninstall.ts source does NOT import 'child_process' or 'node:child_process'",
    () => {
      ...
      assert.ok(
        !importRe.test(src),
        "uninstall.ts must NOT import child_process (fs-only constraint, G-P38.7)",
      );
    },
  );
});
