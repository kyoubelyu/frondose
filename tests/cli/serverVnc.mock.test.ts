...
describe("T-VNC.DES: RFB DES-ECB auth helpers (G-P30.11)", () => {
  it("T-VNC.DES.2: when reverseKeyBits is called on 0x01 (00000001), the result is 0x80 (10000000)", () => {
  ...
  it("T-VNC.DES.2 (edge): reverseKeyBits on 0xAB (10101011) produces 0xD5 (11010101)", () => {
    // Given: 0xAB = 10101011
  ...
  it("T-VNC.DES.1: vncDesResponse with 'testpass' + 16-zero-byte challenge produces a deterministic 16-byte RFC-6143 DES-ECB response", () => {
...
describe("T-VNC.ARD: Apple ARD auth exchange (G-P30.12)", () => {
  it("T-VNC.ARD.1: ardAuthExchange writes encrypted[128] || clientPublic[keyLength] bytes; AES key = MD5(DH shared)", async () => {
    ...
    assert.equal(
      totalWritten,
      256,
      `T-VNC.ARD.1: sock.write must be called with 256 bytes (encrypted+pubKey); got ${totalWritten}`,
    );
  });
});
...
describe("T-VNC.HS: VNC RFB handshake (G-P30.13, G-P30.14)", () => {
  it("T-VNC.HS.1: when target sends RFB type-2 (DES) auth → bridge completes DES → presents type-1 None to browser → pipe established", async () => {
  ...
  it("T-VNC.HS.2: when target offers only type-1 (None), ws is closed with an error; no pipe established", async () => {
...
describe("T-VNC.CFG: VNC config guard (G-P30.15)", () => {
  it("T-VNC.CFG.1: when vnc_port or vnc_password is missing, ws is closed with 'no VNC config for worker X'; createConnection never called", async () => {
