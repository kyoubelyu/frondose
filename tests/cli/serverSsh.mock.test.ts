...
function makeFakeClient(opts: {
  ...
  const fakeStream: FakeStream = Object.assign(new EventEmitter(), {
    writeCalls,
    setWindowCalls,
    write(d: Buffer | string) { writeCalls.push(d); },
    setWindow(rows: number, cols: number) { setWindowCalls.push({ rows, cols }); },
  });
  ...
  return {
    ...
    get connectOpts() { return capturedConnectOpts; },
    fakeStream,
  };
}
...
describe("T-SSH: serverSsh WebSocket bridge (G-P30.8, G-P30.9, G-P30.10)", () => {
  it(
    "T-SSH.1: when handleSshWs called with fake ClientCtor, connect is called; on ready a PTY shell is opened; stream data is forwarded to ws as binary",
    async () => {
  ...
  it(
    "T-SSH.2: when a binary ws message arrives, it is written to the SSH stream",
    async () => {
  ...
  it(
    "T-SSH.3: when a {type:'resize',cols:120,rows:40} JSON text message arrives, stream.setWindow(40,120,0,0) is called",
    async () => {
  ...
  it(
    "T-SSH.4: when the SSH client emits an error, ws receives {type:'error'} message and is closed",
    async () => {
  ...
  it(
    "T-SSH.5: username + port resolution — per-worker JSON > config.ssh_user > os.userInfo().username; agent = SSH_AUTH_SOCK",
    async () => {
      ...
      // Scenario (a): per-worker ssh_user='alice' (passed via target.user)
      {
        ...
        const ws = makeFakeWs();
        handleSshWs(ws as unknown as import("ws").WebSocket, { workerId: "w1", host: "h", user: "alice", port: 22 }, { ClientCtor: fakeCA.ctor });
      ...
      // Scenario (b): config ssh_user=null → OS username
      {
        ...
        handleSshWs(ws as unknown as import("ws").WebSocket, { workerId: "w2", host: "h", user: osUser, port: 22 }, { ClientCtor: fakeCB.ctor });
        await new Promise((r) => setTimeout(r, 30));
      ...
      // Scenario (c): custom port 2222
      {
        ...
        const ws = makeFakeWs();
        handleSshWs(ws as unknown as import("ws").WebSocket, { workerId: "w3", host: "h", user: "u", port: 2222 }, { ClientCtor: fakeCC.ctor });
        await new Promise((r) => setTimeout(r, 30));
