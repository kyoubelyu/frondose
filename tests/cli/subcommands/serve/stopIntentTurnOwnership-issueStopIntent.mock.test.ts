...
import { handlePostAgentAbort, handlePostAgentTurn } from "../../../../src/app/backend/routes/agent.js";
...
describe("generation-safe server turn ownership", () => {
  it("T-Stop.Owner.0: ownership helper clears only its own current turn generation", async () => {
    ...
    const modulePath = new URL("../../../../src/app/backend/turnOwnership.ts", import.meta.url);
...
describe("generation-safe CDP abort-signal ownership", () => {
  ...
  it("T-Stop.Owner.3: session and runOne production sources use owner-aware clear, never unconditional undefined", async () => {
    ...
    const runOnePath = new URL("../../../../src/app/backend/turn/runOne.ts", import.meta.url);
  ...
  it("T-Stop.Owner.4: every delayed top-level turn owner uses CAS cleanup; only two explicit force-releases remain", () => {
    ...
    const files = [
      "../../../../src/app/backend/routes/agent.ts",
      "../../../../src/app/backend/cron.ts",
      "../../../../src/app/backend/dispatch.ts",
      "../../../../src/app/backend/turn/triggers.ts",
    ].map((relative) => ({
      relative,
    ...
    assert.deepEqual(assignmentCounts, [
      { relative: "../../../../src/app/backend/routes/agent.ts", count: 2 },
      { relative: "../../../../src/app/backend/cron.ts", count: 0 },
      { relative: "../../../../src/app/backend/dispatch.ts", count: 0 },
      { relative: "../../../../src/app/backend/turn/triggers.ts", count: 0 },
    ]);
    ...
    assert.deepEqual(helperCallCounts, [
      { relative: "../../../../src/app/backend/routes/agent.ts", count: 3 },
      { relative: "../../../../src/app/backend/cron.ts", count: 1 },
      { relative: "../../../../src/app/backend/dispatch.ts", count: 1 },
      { relative: "../../../../src/app/backend/turn/triggers.ts", count: 1 },
    ]);
  });
  ...
  it("T-Stop.Owner.5: lifecycle formatter is stable and the real route emits one accepted/aborted record per owner", async () => {
    ...
    const lifecyclePath = new URL("../../../../src/app/backend/turnLifecycle.ts", import.meta.url);
    const { formatTurnLifecycleRecord, writeTurnLifecycle } = (await import(lifecyclePath.href)) as {
      ...
      writeTurnLifecycle(event: "accepted" | "aborted", turnId: string, write?: (line: string) => void): void;
    };
    ...
    const agentSource = readFileSync(new URL("../../../../src/app/backend/routes/agent.ts", import.meta.url), "utf8");
