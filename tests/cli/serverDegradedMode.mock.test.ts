...
describe(
  "serverRepl.ts + serverDaemon.ts: startServerHttp called BEFORE resolveModelOrNull (G-P36.4)",
  () => {
    it(
      ...
      () => {
        ...
        for (const [filePath, label] of [
          ...
          const httpLineIdx = lines.findIndex(
            (l) => l.includes("startServerHttp(") && !l.includes("import"),
          );
          // Find first resolveModelOrNull call (not import declaration)
          const modelLineIdx = lines.findIndex(
            (l) => l.includes("resolveModelOrNull(") && !l.includes("import"),
          );

          assert.ok(
            httpLineIdx !== -1,
            `T-FB.3: ${label} must call startServerHttp(...) — not found`,
          );
          assert.ok(
            modelLineIdx !== -1,
            `T-FB.3: ${label} must call resolveModelOrNull() — not found`,
          );
          assert.ok(
...
describe(
  "serverRepl.ts + serverDaemon.ts: agent loop inside `if (model)` guard; else-branch logs disabled message (G-P36.6)",
  () => {
    it(
      ...
      () => {
        ...
        for (const [filePath, label] of [
          ...
          assert.ok(
            ...
            `T-FB.4: ${label} must contain 'if (model)' guard`,
          );
          ...
          assert.ok(
            disabledIdx !== -1,
            `T-FB.4: ${label} must contain '${DISABLED_MSG}'`,
          );
...
describe(
  "serverRepl.ts + serverDaemon.ts: resolveModelOrNull is the sole model-resolution call (G-P36.6)",
  () => {
    it(
      ...
      () => {
        ...
        for (const [filePath, label] of [
          ...
          assert.ok(
            content.includes("resolveModelOrNull"),
            `T-FB.5: ${label} must use resolveModelOrNull`,
          );
