...
describe("B6+B8: BOUNDARY contains both directives (G-P37.9 + G-P37.10)", () => {
  it(
    "T-B68.1: BOUNDARY contains the no-silent-exit directive AND the reply-English directive",
    () => {
...
describe("B6+B8: SERVER_BOUNDARY contains both directives (G-P37.9 + G-P37.10)", () => {
  it(
    "T-B68.2: SERVER_BOUNDARY contains the no-silent-exit directive AND the reply-English directive",
    () => {
...
describe("B6+B8: 3-band composition order unchanged; CHECKPOINT content stable (G-P37.12)", () => {
  it(
    "T-B68.3: composeSystemPrompt band order is Boundary→Soul→Checkpoint (invariant); CHECKPOINT export is non-empty and unchanged",
    () => {
      ...
      assert.equal(composed.indexOf(SEPARATOR, secondSep + 1), -1, "composed must contain exactly 2 separators (not more)");
