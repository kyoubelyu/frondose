...
describe("BOUNDARY constant — single P-57d tool-preference paragraph covering both vision + search (G-P57d.2 + G-P57d.4)", () => {
  it("T-Boundary.1: given BOUNDARY import, WHEN substring + count searches applied, THEN string contains EXACTLY ONE occurrence of 'Tool-preference hints (P-57d scope lock)' header; that paragraph contains BOTH 'analyze_screenshot' + 'inspect' (vision directive) AND 'web_search' + 'MCP_SEARCH_URL' (search directive)", () => {
    // Given: import { BOUNDARY }
    assert.ok(typeof BOUNDARY === "string" && BOUNDARY.length > 0, "BOUNDARY must be non-empty exported string");

    // EXACTLY ONE occurrence of the P-57d paragraph header
    const matches = BOUNDARY.match(/Tool-preference hints \(P-57d scope lock\)/g) ?? [];
    assert.equal(
      matches.length,
      1,
      `BOUNDARY must contain EXACTLY ONE 'Tool-preference hints (P-57d scope lock)' header; got ${matches.length}`,
    );

    // Extract the paragraph after the header (single line per source — header is part of the paragraph body)
    // Source: "**Tool-preference hints (P-57d scope lock).** Vision/screenshot analysis: ..."
    // Both directives are inline in the same string section — verify all 4 keywords coexist.
    const headerIdx = BOUNDARY.indexOf("Tool-preference hints (P-57d scope lock)");
    ...
    assert.ok(remaining.includes("analyze_screenshot"), "P-57d paragraph must contain 'analyze_screenshot'");
    assert.ok(remaining.includes("inspect"), "P-57d paragraph must contain 'inspect'");
    assert.ok(remaining.includes("web_search"), "P-57d paragraph must contain 'web_search'");
    assert.ok(remaining.includes("MCP_SEARCH_URL"), "P-57d paragraph must contain 'MCP_SEARCH_URL'");
    assert.ok(remaining.includes("scope_disabled"), "P-57d paragraph must contain 'scope_disabled'");
    assert.ok(remaining.includes("vision_unavailable"), "P-57d paragraph must contain 'vision_unavailable'");
  });
});
