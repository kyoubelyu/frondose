...
switch (mutation) {
  case "missing-exe":
    rmSync(path("Frondose-windows-x86_64-setup.exe"));
    break;
  case "missing-exe-sig":
    rmSync(path("Frondose-windows-x86_64-setup.exe.sig"));
    break;
  case "renamed-exe":
    renameSync(path("Frondose-windows-x86_64-setup.exe"), path("Frondose-renamed.nsis.exe"));
    break;
  case "renamed-exe-sig":
    renameSync(path("Frondose-windows-x86_64-setup.exe.sig"), path("Frondose-renamed.nsis.exe.sig"));
    break;
  case "substituted-exe":
    writeFileSync(path("Frondose-windows-x86_64-setup.exe"), "different installer bytes\n");
    break;
  case "foreign-exe-sig":
    copyFileSync(path("Frondose.app.tar.gz.sig"), path("Frondose-windows-x86_64-setup.exe.sig"));
    break;
  case "foreign-mac-sig":
    copyFileSync(path("Frondose-windows-x86_64-setup.exe.sig"), path("Frondose.app.tar.gz.sig"));
    break;
  ...
  case "missing-checksum": {
    ...
    writeFileSync(path("SHA256SUMS"), `${lines.filter((line) => !line.endsWith("  Frondose-windows-x86_64-setup.exe")).join("\n")}`);
    break;
  }
  case "missing-provenance": {
    const provenance = JSON.parse(readFileSync(path("provenance.json"), "utf8"));
    provenance.subject = provenance.subject.filter((subject) => subject.name !== "Frondose-windows-x86_64-setup.exe");
  ...
  case "zip-reintroduced":
    writeFileSync(path("Frondose.nsis.zip"), readFileSync(path("Frondose-windows-x86_64-setup.exe")));
    break;
  default:
