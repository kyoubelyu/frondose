...
describe("workerSubSchema input_mode — G-P32.1", () => {
  it(
    'T-CFG.1: when config.json has worker.input_mode:"hardware", readConfig().worker.input_mode === "hardware"',
    () => {
      ...
      try {
        writeFileSync(
          configPath,
          JSON.stringify({ schema_version: 2, worker: { input_mode: "hardware" } }),
          "utf-8",
        );
  ...
  it(
    "T-CFG.2: when config.json has no input_mode field, readConfig().worker.input_mode === \"cdp\" (Zod default)",
    () => {
  ...
  it(
    'T-CFG.3: when input_mode is an invalid value "bogus", readConfig returns default config (Zod parse-fail path) with stderr warning',
    () => {
      ...
      try {
        writeFileSync(
          configPath,
          JSON.stringify({ schema_version: 2, worker: { input_mode: "bogus" } }),
          "utf-8",
        );
        const result = readConfig(configPath);
