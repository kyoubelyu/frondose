...
describe("T-SP-C.Passive — buildPassivePrompt Magical-mode profile-nav branch (P-SP-C)", () => {
  ...
  // ─── T-SP-C.Passive.1 ────────────────────────────────────────────────────────
  it("T-SP-C.Passive.1: profile-nav prompt source contains kernel-tool sequence keywords + candidateId + NO outbound imperatives (D-SP-C-A: source-structural scan since export missing)", () => {
    ...
    if (typeof buildPassivePrompt === "function") {
      ...
      for (const keyword of [
        ...
        "candidateId",
        "alice-vp",
        "https://www.linkedin.com/in/alice-vp/",
      ]) {
        assert.ok(prompt.includes(keyword), `profile-nav prompt must contain "${keyword}"`);
      }
      assert.ok(
        prompt.includes("NEVER call") || prompt.includes("NEVER"),
      ...
      for (const keyword of ["record_raw_candidate", "search_memory", "score_lead", "suggest_card", "candidateId"]) {
        assert.ok(
          profileNavSection.includes(keyword),
  ...
  // ─── T-SP-C.Passive.2 ────────────────────────────────────────────────────────
  it("T-SP-C.Passive.2: profile-nav prompt instructs agent to thread candidateId from step 1 into step 3 (D-SP-C-A: source-structural scan)", () => {
  ...
  // ─── T-SP-C.Passive.3 ────────────────────────────────────────────────────────
  it("T-SP-C.Passive.3: click + input branches UNCHANGED — source still contains 'remember' and 'stop' in those branches", () => {
    ...
    if (typeof buildPassivePrompt === "function") {
      ...
      assert.ok(clickBranchOffset >= 0, 'buildPassivePrompt must contain click branch `if (eventType === "click")`');
      ...
      assert.ok(
        clickSection.includes("remember"),
        `passive.ts buildPassivePrompt click branch must contain 'remember'. ` +
          `Snippet: ${clickSection.slice(0, 400)}`,
      );
      assert.ok(clickSection.includes("stop"), "passive.ts buildPassivePrompt click branch must contain 'stop'");

      // Input branch within buildPassivePrompt
      const inputBranchOffset = fnBody.indexOf('if (eventType === "input")');
      assert.ok(inputBranchOffset >= 0, 'buildPassivePrompt must contain input branch `if (eventType === "input")`');
      const inputSection = fnBody.slice(inputBranchOffset, inputBranchOffset + 800);
      assert.ok(
        inputSection.includes("remember"),
        "passive.ts buildPassivePrompt input branch must contain 'remember'",
      );
      assert.ok(inputSection.includes("stop"), "passive.ts buildPassivePrompt input branch must contain 'stop'");
    }
  });
});
