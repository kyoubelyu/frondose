...
describe("tool count freeze — G-P32.21 / D-6", () => {
  it("T-CONTRACT.TOOLS: makeAllTools worker mode returns 52 tools; server mode returns 25 tools (P-73 rebaseline)", () => {
    ...
    try {
      ...
      assert.equal(workerCount, 52, `T-CONTRACT.TOOLS: worker tool count must be 52, got ${workerCount}`);
