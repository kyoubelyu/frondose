...
describe("no-bash boundary — P-32 new/edited files (G-P32.21)", () => {
  it(
    "T-CONTRACT.NO-BASH: grep child_process across P-32 new/edited src files → zero hits",
    () => {
      ...
      assert.equal(
        output,
        "",
        `T-CONTRACT.NO-BASH: child_process found in P-32 files:\n${output}`,
      );
    },
  );
});
...
describe("tool count freeze — G-P32.21 / D-6", () => {
  it(
    "T-CONTRACT.TOOLS: makeAllTools worker mode returns 28 tools; server mode returns 19 tools (unchanged from pre-P-32)",
    () => {
      ...
      try {
        ...
        assert.equal(workerCount, 28, `T-CONTRACT.TOOLS: worker tool count must be 28, got ${workerCount}`);
        assert.equal(serverCount, 19, `T-CONTRACT.TOOLS: server tool count must be 19, got ${serverCount}`);
      } finally {
        cleanup();
      }
    },
...
describe("cgevent.ts loader — G-P32.20", () => {
  it(
    "T-CONTRACT.LOADER: loadCgEvent() loads addon when .node present; throws clear Error when absent",
    async () => {
      ...
      if (existsSync(nodePath)) {
        ...
        assert.ok(typeof cgAny === "object" && cgAny !== null,
          "T-CONTRACT.LOADER: loadCgEvent must return a non-null object when .node is present");
        assert.ok(typeof cgAny.moveMouse === "function",
          "T-CONTRACT.LOADER: addon must export moveMouse");
        assert.ok(typeof cgAny.mouseClick === "function",
          "T-CONTRACT.LOADER: addon must export mouseClick");
        assert.ok(typeof cgAny.unicodeType === "function",
          "T-CONTRACT.LOADER: addon must export unicodeType");
        assert.ok(typeof cgAny.isAccessibilityTrusted === "function",
          "T-CONTRACT.LOADER: addon must export isAccessibilityTrusted");
        assert.ok(typeof cgAny.scrollWheel === "function",
          "T-CONTRACT.LOADER: addon must export scrollWheel");
        assert.ok(typeof cgAny.keyEvent === "function",
          "T-CONTRACT.LOADER: addon must export keyEvent");
        assert.ok(typeof cgAny.getMousePos === "function",
          "T-CONTRACT.LOADER: addon must export getMousePos");
        assert.ok(typeof cgAny.getScreenSize === "function",
          "T-CONTRACT.LOADER: addon must export getScreenSize");
      } else {
...
describe("N-API arity guard — CONCERN-2 (guardian §3) — skip if .node absent", () => {
  it(
    "T-CONTRACT.ARITY: each addon exported function guards info.Length() and throws TypeError on wrong arity",
    async () => {
