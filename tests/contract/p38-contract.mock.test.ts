...
const FROZEN_RELEASE_YML = readFileSync(
...
describe(".gitignore no longer blocks .github/ (G-P38.1)", () => {
  it(
    "T-GI.1: .gitignore does NOT contain a '.github/' entry (the CI/CD block is removed)",
    () => {
...
describe(".github/workflows/release.yml is git-tracked (G-P38.2)", () => {
  it(
    "T-GI.2: 'git ls-files .github/workflows/release.yml' returns the path (file is tracked)",
    () => {
...
describe("release.yml content is byte-identical to pre-P-38 file (G-P38.2)", () => {
  it(
    "T-RY.1: .github/workflows/release.yml content after P-38 equals the frozen pre-P-38 snapshot (ship-as-is — no edit)",
    () => {
...
describe("no child_process in P-38's edited .ts files (G-P38.7/.11)", () => {
  it(
    "T-CONTRACT.NO-BASH: P-38's edited TypeScript files contain no direct 'child_process' import",
    () => {
      ...
      for (const filePath of p38TsFiles) {
        ...
        assert.ok(
          ...
          `${filePath} must NOT import child_process directly (G-P38.7 / G-P38.11)`,
        );
      }
    },
  );
...
describe("backfill-releases.sh — syntax-valid + correct tag range + idempotent (G-P38.10)", () => {
  it(
    "T-BACKFILL.1: backfill-releases.sh exists at repo root; bash -n passes; tag list is exactly v0.4.26..v0.4.34; uses 'gh release create' + skip-on-existing guard",
    () => {
      ...
      const expectedTags = [
        "v0.4.26", "v0.4.27", "v0.4.28", "v0.4.29", "v0.4.30",
        "v0.4.31", "v0.4.32", "v0.4.33", "v0.4.34",
      ];
      for (const tag of expectedTags) {
        assert.ok(
          scriptContent.includes(tag),
          `backfill-releases.sh must contain tag ${tag} (G-P38.10)`,
        );
      }
