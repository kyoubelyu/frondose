 ...
 * src/overlay/, src/app/backend/, src/linkedin/, and scripts/. They will
...
describe("G-FREN5.1 — zero mai survivors in overlay/serve/linkedin/scripts after rename", () => {
  it("T-FREN5.1: when rename is complete, grep for __mai|mai-overlay|data-mai-*|MAI_OVERLAY|MAI_PASSIVE|MAI_DIALOG|MAI_OUTPUT|MAI_FRAMES|__MAI_|maiOverlay|maiReadDialogState|maiWriteDialogState|maiDialogState|__maiShared → ZERO hits", () => {
    ...
    const BANNED =
      /(__mai|mai-overlay|data-mai-(ov|pm|pa|rg)|MAI_OVERLAY|MAI_PASSIVE|MAI_DIALOG|MAI_OUTPUT|MAI_FRAMES|__MAI_|maiOverlay|\bmaiReadDialogState\b|\bmaiWriteDialogState\b|\bmaiDialogState\b|__maiShared)/;
...
describe("G-FREN5.2 — no orphan serve-layer RPC caller", () => {
  it("T-FREN5.2: when all RPC definitions exist in bootstrap*.ts, every callInOverlay/RPC string in serve/**+session.ts uses a defined __frondoseX name → caller-set ⊆ definition-set", () => {
  ...
  it("T-FREN5.2b: zero window.__maiX( callers survive in serve/**+session.ts (sanity complement of G-FREN5.1)", () => {
...
describe("G-FREN5.3 — binding-name parity (host.ts addBinding ↔ eventBus.ts bindingCalled filter)", () => {
  it('T-FREN5.3: when rename is complete, host.ts addBinding({name:"..."}) and eventBus.ts name !== "..." both equal "__frondosePost" and neither contains "__maiPost"', () => {
    ...
    assert.ok(busMatch, 'G-FREN5.3 FAIL: eventBus.ts name !== "..." pattern not found');
    assert.equal(
      busMatch[1],
      "__frondosePost",
    ...
    assert.ok(!/__maiPost/.test(hostText), 'G-FREN5.3 FAIL: host.ts still contains "__maiPost"');
    assert.ok(!/__maiPost/.test(eventBusText), 'G-FREN5.3 FAIL: eventBus.ts still contains "__maiPost"');
  });
});
...
describe("G-FREN5.4 — placeholder parity and guard: bootstrap.ts emits __FRONDOSE_*__ ↔ host.ts substitutes and guards them", () => {
  it("T-FREN5.4a: bootstrap.ts emits __FRONDOSE_OVERLAY_OWNER__, __FRONDOSE_OVERLAY_VERSION__, __FRONDOSE_PASSIVE_ENABLED__ as placeholder RHS values", () => {
    ...
    assert.ok(!text.includes("__MAI_OVERLAY_OWNER__"), "G-FREN5.4a FAIL: bootstrap.ts still has __MAI_OVERLAY_OWNER__");
    assert.ok(
  ...
  it("T-FREN5.4b: host.ts .replace() regexes reference __FRONDOSE_*__ tokens (not __MAI_*__), and the guard string references the frondose tokens", () => {
    ...
    const guardOwner =
      text.includes("__FRONDOSE_OVERLAY_OWNER__") &&
      /substituted\.includes\(["']__FRONDOSE_OVERLAY_OWNER__["']\)/.test(text);
    const guardVersion =
      text.includes("__FRONDOSE_OVERLAY_VERSION__") &&
      /substituted\.includes\(["']__FRONDOSE_OVERLAY_VERSION__["']\)/.test(text);
    assert.ok(guardOwner, "G-FREN5.4b FAIL: host.ts guard does not reference __FRONDOSE_OVERLAY_OWNER__");
    assert.ok(guardVersion, "G-FREN5.4b FAIL: host.ts guard does not reference __FRONDOSE_OVERLAY_VERSION__");

    // Old tokens must be absent from host.ts
    assert.ok(!text.includes("__MAI_OVERLAY_OWNER__"), "G-FREN5.4b FAIL: host.ts still has __MAI_OVERLAY_OWNER__");
    assert.ok(!text.includes("__MAI_OVERLAY_VERSION__"), "G-FREN5.4b FAIL: host.ts still has __MAI_OVERLAY_VERSION__");
    assert.ok(!text.includes("__MAI_PASSIVE_ENABLED__"), "G-FREN5.4b FAIL: host.ts still has __MAI_PASSIVE_ENABLED__");
  });

  it("T-FREN5.4c: placeholder TOKEN set in bootstrap.ts ⊆ replacement regex set in host.ts (no unresolved leftover possible)", () => {
...
describe("G-FREN5.5 — generated bundle: sharedRenderBundle.generated.ts contains __frondoseShared and zero __maiShared; gen-overlay-assets.ts globalName = __frondoseShared", () => {
  it("T-FREN5.5a: sharedRenderBundle.generated.ts contains 'var __frondoseShared' and zero '__maiShared' after npm run build:overlay-assets", () => {
    ...
    const text = readSrc("sharedRenderBundle.generated.ts", IN_SCOPE_FILES["sharedRenderBundle.generated.ts"]!);
  ...
  it("T-FREN5.5b: gen-overlay-assets.ts globalName literal is '__frondoseShared' and its guard string is '__frondoseShared'", () => {
    ...
    assert.ok(globalNameMatch, "G-FREN5.5b FAIL: gen-overlay-assets.ts globalName: '...' pattern not found");
    assert.equal(
      globalNameMatch[1],
      "__frondoseShared",
    ...
    // Old name absent
    assert.ok(!text.includes("__maiShared"), 'G-FREN5.5b FAIL: gen-overlay-assets.ts still contains "__maiShared"');
  });

  it("T-FREN5.5c: bootstrapShell.ts uses __frondoseShared.X (not __maiShared.X) for all 6 consumer sites (lines 26,55,185,190,204,208)", () => {
...
describe("G-FREN5.6 — worldName parity: host.ts addScriptToEvaluateOnNewDocument worldName and context.name filter both = 'frondose-overlay'", () => {
  it("T-FREN5.6: when rename is complete, host.ts worldName literal and context.name !== filter both equal 'frondose-overlay'; neither contains 'mai-overlay'", () => {
    ...
    assert.ok(worldMatch, "G-FREN5.6 FAIL: host.ts worldName: '...' pattern not found");
    assert.equal(
      worldMatch[1],
      "frondose-overlay",
    ...
    assert.ok(filterMatch, "G-FREN5.6 FAIL: host.ts context.name !== '...' filter pattern not found");
    assert.equal(
      filterMatch[1],
      "frondose-overlay",
    ...
    // Old value absent
    assert.ok(!text.includes("mai-overlay"), 'G-FREN5.6 FAIL: host.ts still contains "mai-overlay"');
  });
});
...
describe("G-FREN5.7 — element-ID set⇄selector parity: #__frondose_root and #__frondose_collapsed_card in both assignment and selector; no #__mai_* selectors", () => {
  it("T-FREN5.7a: bootstrap.ts assigns ids __frondose_root and __frondose_collapsed_card; no __mai_root or __mai_collapsed_card id strings remain", () => {
    ...
    assert.ok(!text.includes("__mai_root"), "G-FREN5.7a FAIL: bootstrap.ts still has '__mai_root'");
    assert.ok(!text.includes("__mai_collapsed_card"), "G-FREN5.7a FAIL: bootstrap.ts still has '__mai_collapsed_card'");
  });

  it("T-FREN5.7b: bootstrapLegacy.ts uses '#__frondose_root' and '#__frondose_collapsed_card' as closest() selectors; zero '#__mai_*' selectors remain", () => {
    ...
    assert.ok(!text.includes("#__mai_root"), "G-FREN5.7b FAIL: bootstrapLegacy.ts still has '#__mai_root' selector");
    assert.ok(
...
describe("G-FREN5.8 — region-tag set==query per module: data-frondose-ov/pm/pa in snapshotCapture.ts; data-frondose-rg-aside in regionTag.ts; no data-mai-*", () => {
  it("T-FREN5.8a: snapshotCapture.ts: data-frondose-ov setAttribute/setAttribute string AND querySelectorAll selector are both present; data-mai-ov is absent", () => {
    ...
    assert.ok(text.includes("data-frondose-ov"), "G-FREN5.8a FAIL: snapshotCapture.ts missing 'data-frondose-ov'");
    assert.ok(!text.includes("data-mai-ov"), "G-FREN5.8a FAIL: snapshotCapture.ts still has 'data-mai-ov'");
  });

  it("T-FREN5.8b: snapshotCapture.ts: data-frondose-pm setAttribute/querySelectorAll present; data-mai-pm absent", () => {
    ...
    assert.ok(text.includes("data-frondose-pm"), "G-FREN5.8b FAIL: snapshotCapture.ts missing 'data-frondose-pm'");
    assert.ok(!text.includes("data-mai-pm"), "G-FREN5.8b FAIL: snapshotCapture.ts still has 'data-mai-pm'");
  });

  it("T-FREN5.8c: snapshotCapture.ts: data-frondose-pa setAttribute/querySelectorAll present; data-mai-pa absent", () => {
    ...
    assert.ok(text.includes("data-frondose-pa"), "G-FREN5.8c FAIL: snapshotCapture.ts missing 'data-frondose-pa'");
    assert.ok(!text.includes("data-mai-pa"), "G-FREN5.8c FAIL: snapshotCapture.ts still has 'data-mai-pa'");
  });

  it("T-FREN5.8d: regionTag.ts: data-frondose-rg-aside setAttribute/querySelectorAll present; data-mai-rg-aside absent", () => {
    ...
    assert.ok(!text.includes("data-mai-rg-aside"), "G-FREN5.8d FAIL: regionTag.ts still has 'data-mai-rg-aside'");
  });

  it("T-FREN5.8e: within each module, the setAttribute tag-value string equals the querySelectorAll selector base-attribute — set==query parity per-module", () => {
    ...
    for (const tag of ["data-frondose-ov", "data-frondose-pm", "data-frondose-pa"]) {
      ...
      assert.ok(inSetAttr, `G-FREN5.8e FAIL: snapshotCapture.ts: '${tag}' not found as a setAttribute argument`);
      assert.ok(inSelector, `G-FREN5.8e FAIL: snapshotCapture.ts: '[${tag}' not found as a querySelectorAll selector`);
    }
...
describe("G-FREN5.9 — Tauri UI untouched: src/tauri/ui/ has zero __mai[A-Za-z] hits (was 0 before; downstream-of-binding invariant preserved)", () => {
  it("T-FREN5.9: src/tauri/ui/**/*.ts grep for __mai[A-Za-z] → zero hits (invariant holds regardless of rename)", () => {
...
describe("G-FREN5.3 addendum — T-FREN5.3b: in-page binding callers in bootstrap.ts + bootstrapLegacy.ts call exactly __frondosePost (not any other new name)", () => {
  it("T-FREN5.3b: when rename complete, every in-page window.__frondose*Post( call in bootstrap.ts + bootstrapLegacy.ts names exactly __frondosePost; total in-page post-call count is 3", () => {
...
describe("G-FREN5.2 addendum — T-FREN5.2c: in-page RPC callers in bootstrap fragments have matching definitions in bootstrap definition set", () => {
  it("T-FREN5.2c: when rename complete, every non-assignment window.__frondoseX( call in bootstrap*.ts (excluding __frondosePost) has a matching window.__frondoseX = definition", () => {
    ...
    for (const fname of BOOTSTRAP_FILES) {
      ...
      for (const [lineIdx, line] of lines.entries()) {
        ...
        for (const m of line.matchAll(CALLER_RE)) {
          ...
          if (!definitionSet.has(name)) {
            violations.push(
              `  ${rel}:${lineIdx + 1}  orphan in-page caller '${name}' — no definition in bootstrap*.ts`,
            );
          }
        }
      }
...
describe("G-FREN5.7 addendum — T-FREN5.7c: dataset.frondoseOverlay{Owner,Version,Stale} write↔read parity in bootstrap.ts; zero dataset.maiOverlay* remain", () => {
  it("T-FREN5.7c: bootstrap.ts: frondoseOverlayOwner + frondoseOverlayVersion each appear in both write (.dataset.X =) and read (.dataset.X) roles; frondoseOverlayStale appears at stale-marker write; zero dataset.maiOverlay* remain", () => {
    ...
    assert.ok(writeVersion, "T-FREN5.7c FAIL: bootstrap.ts missing `.dataset.frondoseOverlayVersion =` write");
    assert.ok(writeStale, "T-FREN5.7c FAIL: bootstrap.ts missing `.dataset.frondoseOverlayStale =` stale-marker write");
    assert.ok(
      readOwner,
    ...
    assert.ok(readVersion, "T-FREN5.7c FAIL: bootstrap.ts missing `.dataset.frondoseOverlayVersion` read");
...
describe("G-FREN5.8 addendum — T-FREN5.8e cleanup parity: each region tag appears in a cleanup site (removeAttribute or cleanup querySelectorAll) in its module", () => {
  it("T-FREN5.8e-cleanup: for each of data-frondose-ov, -pm, -pa (snapshotCapture.ts) and data-frondose-rg-aside (regionTag.ts), the tag also appears in a cleanup removeAttribute or cleanup querySelectorAll context", () => {
    ...
    for (const tag of ["data-frondose-ov", "data-frondose-pm", "data-frondose-pa"]) {
      const hasCleanupRemove =
        scText.includes(`removeAttribute('${tag}')`) || scText.includes(`removeAttribute("${tag}")`);
    ...
    const hasRgCleanupRemove =
      rtText.includes("removeAttribute('data-frondose-rg-aside')") ||
      rtText.includes('removeAttribute("data-frondose-rg-aside")');
...
describe("G-FREN5.7 addendum — T-FREN5.7a-cron-stale: __frondose_root_stale_ prefix and __frondose_cron_banner id appear in bootstrap.ts + bootstrapLegacy.ts", () => {
  it("T-FREN5.7a-cron-stale: bootstrap.ts contains __frondose_root_stale_ prefix (:38) and __frondose_cron_banner id (:50); bootstrapLegacy.ts contains __frondose_cron_banner (:288)", () => {
...
describe("G-FREN5.11 — typecheck + lint + build clean (marker gate; npm run check at Step 5)", () => {
  it("T-FREN5.11: [Step-5 deferred] tsconfig.json and biome.json exist, confirming check/lint infrastructure is present for Step-5 command verification", () => {
    ...
    try {
      tsconfigStat = statSync(tsconfigPath);
    } catch {
      /* */
    }
...
describe("G-FREN5.10 / mai-pill guard — frondose-pill is present in bootstrapShell.ts + cssTransform.ts + frondoseCss.generated.ts; zero mai-pill remains", () => {
  it("T-FREN5.PILL: when orchestrator-folded mai-pill→frondose-pill rename is complete, all three pill-bearing files use 'frondose-pill' className/selector and zero 'mai-pill' survives", () => {
