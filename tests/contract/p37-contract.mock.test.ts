...
describe("no child_process import in P-37's 7 edited production files (G-P37.12)", () => {
  it(
    "T-CONTRACT.NO-BASH: none of P-37's 7 in-scope production files contain 'child_process' import",
    () => {
...
describe("tool counts: worker 29 / server 20 unchanged across P-37 (G-P37.12)", () => {
  it(
    "T-CONTRACT.TOOLS: P-37 does not add or remove any tool from makeAllTools (worker 29 / server 20)",
    () => {
      // Given: makeAllTools called in worker mode and server mode with fake deps
      // When:  count the tool registrations returned
      // Then:  worker count === 29; server count === 20 (unchanged from P-36 baseline)
...
describe("hardwareScroll byte-unchanged by P-37 (G-P37.13)", () => {
  it(
    "T-CONTRACT.HW: hardwareInput.ts hardwareScroll uses amount-based cg.scrollWheel (no synthesizeScrollGesture); P-37 did not touch it",
    () => {
