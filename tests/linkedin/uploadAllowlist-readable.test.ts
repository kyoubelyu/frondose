 ...
 * T-Sandbox.6 — FRONDOSE_UPLOAD_ALLOWLIST path → allowed
...
// ─── T-Sandbox.6: FRONDOSE_UPLOAD_ALLOWLIST custom dir → allowed ──────────────────

test("T-Sandbox.6: path inside FRONDOSE_UPLOAD_ALLOWLIST custom dir → allowed", () => {
  const customDir = mkdtempSync(join(tmpdir(), "mai-p9-allowlist-"));
  try {
    const allowedPath = join(customDir, "screenshot.png");
    withEnv("FRONDOSE_UPLOAD_ALLOWLIST", customDir, () => {
    ...
    withEnv("FRONDOSE_UPLOAD_ALLOWLIST", undefined, () => {
      ...
      // So this test only verifies the FRONDOSE_UPLOAD_ALLOWLIST mechanism works
