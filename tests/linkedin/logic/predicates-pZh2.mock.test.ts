...
import { FEED_COMPOSER_EDITOR_JS } from "../../../src/linkedin/composerReadiness.js";
...
describe("T-Detect.1 — composer input JS pattern is ZH-aware", () => {
  it("T-Detect.1: the generated composer-editor JS string's INPUT_RE contains the ZH placeholder token", () => {
    // Given: the two composer-editor JS builders (labelPattern-driven and hardcoded-literal).
    // When:  each is generated with its default/current pattern.
    // Then:  the generated source must contain the ZH placeholder token — it does NOT today
    //   (both are English-only), which is the genuine pre-Step-4 RED.
...
describe("T-Detect.4 — thread composer input detection is ZH-aware", () => {
  it("T-Detect.4: isThreadComposerInputEntry matches '写消息'/'消息'; MESSAGING_COMPOSER_SYNTH_JS's INPUT_RE embeds the ZH tokens", () => {
    ...
    assert.equal(isThreadComposerInputEntry(entry("textbox", "写消息", "@e1")), true, 'isThreadComposerInputEntry("写消息") must be true');
    assert.equal(isThreadComposerInputEntry(entry("textbox", "消息", "@e2")), true, 'isThreadComposerInputEntry("消息") must be true');
    assert.ok(
