...
describe("click tool branching — G-P32.7", () => {
  it("T-BR.CLICK.1: when session.inputMode='hardware', click tool routes to hardwareClickAt (client.clickAt NOT called)", async () => {
  ...
  it("T-BR.CLICK.2: when session.inputMode='cdp', click tool calls client.clickAt (hardware path NOT taken); envelope unchanged", async () => {
...
describe("type tool branching — G-P32.8", () => {
  it("T-BR.TYPE.1: when session.inputMode='hardware', type tool routes to hardwareTypeAt (client.clickAt NOT called for CDP path)", async () => {
    // Given: session.inputMode="hardware"; fake CdpClient with clickAt spy
...
describe("scroll tool branching — G-P32.9", () => {
  it("T-BR.SCROLL.1: when session.inputMode='hardware', scroll tool routes to hardwareScroll (client.scroll NOT called for CDP path)", async () => {
...
describe("press tool branching — G-P32.10", () => {
  it("T-BR.PRESS.1: when session.inputMode='hardware', press tool routes to hardwarePressKey (dispatchKeyEvent NOT called)", async () => {
    ...
    (client as any).handle.Input.dispatchKeyEvent = async () => {
      dispatchCalled = true;
    };
    const session = makeHardwareSession(client);
...
describe("tool parameter schema freeze — G-P32.22", () => {
  it("T-BR.SCHEMA.1: click/type/scroll/press tool parameters schemas are structurally unchanged from pre-P-32", () => {
