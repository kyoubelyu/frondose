...
describe("install.sh bash syntax check (G-P40.8)", () => {
  it(
    "T-Struct.1: when 'bash -n install.sh' is run, exit code is 0 (no syntax errors)",
    () => {
...
describe("install.sh shellcheck lint (G-P40.8)", () => {
  it(
    "T-Struct.2: when 'shellcheck install.sh' is run (if available), no errors reported (disable directives allowed)",
    () => {
...
describe("install.sh — no GITHUB_TOKEN (G-P40.1)", () => {
  it(
    "T-Grep.1: when install.sh non-comment lines are scanned, none reference 'GITHUB_TOKEN'",
    () => {
...
describe("install.sh — gh CLI preflight (G-P40.2)", () => {
  it(
    "T-Grep.2: when install.sh content is read, it contains 'command -v gh' AND 'gh auth status', each associated with 'exit 1'",
    () => {
...
describe("install.sh — gh-native release fetch; no curl/python3/api.github.com (G-P40.3)", () => {
  it(
    "T-Grep.3: install.sh contains 'gh release view' AND 'gh release download'; non-comment lines do NOT contain 'api.github.com', 'python3', or 'curl'",
    () => {
      // Given: install.sh P-40 rewrite
...
describe("install.sh — MAI_PREFIX overrides both base paths (G-P40.4)", () => {
  it(
    "T-Grep.4: install.sh contains '${MAI_PREFIX:-$HOME}' AND '${MAI_PREFIX:-$(brew --prefix)}'",
    () => {
      ...
      const content = readFileSync(INSTALL_SH, "utf8");
      assert.ok(
        content.includes("${MAI_PREFIX:-$HOME}"),
        "HOME_BASE must use '${MAI_PREFIX:-$HOME}'",
      );
      assert.ok(
...
describe("install.sh — no | tail -N output suppression (G-P40.5)", () => {
  it(
    "T-Grep.5: install.sh contains no '| tail -3' or '| tail -5' patterns",
    () => {
...
describe("install.sh — Xcode CLT: WARNING not exit (G-P40.6)", () => {
  it(
    "T-Grep.6: install.sh contains 'xcode-select -p' and a 'WARNING' line; the CLT-absent branch has no bare 'exit' statement",
    () => {
...
describe("install.sh — every gh release command carries --repo kyoubelyu/mai-agent (G-P40.8)", () => {
  it(
    "T-Grep.7: every line in install.sh that contains 'gh release' also contains '--repo' with 'kyoubelyu/mai-agent' (or the $REPO variable bound to it)",
    () => {
      ...
      for (const line of ghReleaseLines) {
        assert.ok(
          line.includes("--repo"),
          `'gh release' line missing '--repo': ${line.trim()}`,
        );
      }
      assert.ok(
...
describe("package.json — scripts.install === 'true' (G-P40.7)", () => {
  it(
    "T-Pkg.1: when package.json is parsed, scripts.install === 'true' (suppresses npm implicit node-gyp rebuild)",
    () => {
...
describe("package.json — node-gyp appears only in build:native (G-P40.7)", () => {
  it(
    "T-Pkg.2: in package.json scripts, 'node-gyp' appears only inside scripts['build:native']; no other script contains 'node-gyp'",
    () => {
