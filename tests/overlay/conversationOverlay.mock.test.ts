...
type OverlayImpl = {
  convList: FakeEl;
  modeBadge: FakeEl;
  __frondoseAppendUser: (text: string) => void;
  __frondoseBeginAgent: () => void;
  __frondoseAppendChunk: (chunk: string) => void;
  __frondoseEndAgent: () => void;
  __frondoseSetMode: (mode: string) => void;
  __frondoseAppendOutput: (chunk: string) => void; // C-1 legacy wrapper
  __frondoseClearOutput: () => void; // C-2 legacy wrapper
...
function makeOverlayImpl(): OverlayImpl {
  ...
  function __frondoseAppendUser(text: string): void {
  ...
  function __frondoseBeginAgent(): void {
    const list = shadow_getElementById("conversation-list");
    if (!list) return;
  ...
  function __frondoseAppendChunk(chunk: string): void {
    // [NEW-BLOCKER-1 fix] Frame-agnostic auto-open: if no active bubble, open one.
    // Mirrors SHELL_JS: if (!activeAgentTextEl && typeof window.__frondoseBeginAgent === 'function')
    if (!activeAgentTextEl) __frondoseBeginAgent();
    if (!activeAgentTextEl) return;
    activeAgentTextEl.textContent = (activeAgentTextEl.textContent || "") + String(chunk);
  }

  function __frondoseEndAgent(): void {
    activeAgentTextEl = null;
  }

  function __frondoseSetMode(mode: string): void {
    const mb = shadow_getElementById("mode-badge");
    if (!mb) return;
    // Mirrors SHELL_JS __frondoseSetMode logic (plan §5.3.4)
  ...
  // C-1 migration: __frondoseAppendOutput routes to __frondoseAppendChunk
  function __frondoseAppendOutput(chunk: string): void {
    __frondoseAppendChunk(chunk);
  }

  // C-2 migration: __frondoseClearOutput routes to __frondoseEndAgent (NO DOM wipe)
  function __frondoseClearOutput(): void {
    __frondoseEndAgent();
  }
  ...
  return {
    convList,
    modeBadge,
    __frondoseAppendUser,
    __frondoseBeginAgent,
    __frondoseAppendChunk,
    __frondoseEndAgent,
    __frondoseSetMode,
    __frondoseAppendOutput,
    __frondoseClearOutput,
...
describe("T-PY2MA.Overlay.1 — __frondoseAppendUser adds .msg-user to #conversation-list (G-PY2MA.6)", () => {
  it("T-PY2MA.Overlay.1: window.__frondoseAppendUser(text) appends <div class='msg-user'> to convList", () => {
    // Given: the overlay is installed; shadow root's #conversation-list is empty
    // When:  window.__frondoseAppendUser("Let's find leads") is called
    // Then:  convList.children[0] exists with className === "msg-user"
    //        and textContent === "Let's find leads"
    const { convList, __frondoseAppendUser } = makeOverlayImpl();

    __frondoseAppendUser("Let's find leads");

    assert.equal(convList.children.length, 1, "convList must have 1 child after __frondoseAppendUser (G6)");
...
describe("T-PY2MA.Overlay.2 — __frondoseBeginAgent appends .msg-agent with sparkles SVG (G-PY2MA.6 + G3)", () => {
  it("T-PY2MA.Overlay.2: window.__frondoseBeginAgent() appends <div class='msg-agent'> with .avatar containing <svg><path d='M12 2.5...'>", () => {
    // Given: convList is empty; activeAgentTextEl === null
    // When:  window.__frondoseBeginAgent() is called
    ...
    const { convList, __frondoseBeginAgent, getActiveEl } = makeOverlayImpl();

    __frondoseBeginAgent();

    assert.equal(convList.children.length, 1, "convList must have 1 msg-agent after __frondoseBeginAgent (G6)");
    ...
    assert.notEqual(getActiveEl(), null, "activeAgentTextEl must be non-null after __frondoseBeginAgent (G6)");
  });
});

// ─── T-PY2MA.Overlay.3 ──────────────────────────────────────────────────────

describe("T-PY2MA.Overlay.3 — __frondoseAppendChunk appends to active agent bubble (G-PY2MA.6)", () => {
  it("T-PY2MA.Overlay.3: window.__frondoseAppendChunk(text) concatenates text into activeAgentTextEl.textContent", () => {
    // Given: __frondoseBeginAgent() was called → activeAgentTextEl is .msg-agent-text (textContent="")
    // When:  __frondoseAppendChunk("Hello ") then __frondoseAppendChunk("world") are called
    // Then:  activeAgentTextEl.textContent === "Hello world"
    //        AND convList still contains exactly ONE .msg-agent bubble
    const { convList, __frondoseBeginAgent, __frondoseAppendChunk, getActiveEl } = makeOverlayImpl();

    __frondoseBeginAgent();
    __frondoseAppendChunk("Hello ");
    __frondoseAppendChunk("world");
...
describe("T-PY2MA.Overlay.5 — __frondoseSetMode('auto') sets badge text to 'AUTO' + adds .auto class (G-PY2MA.7)", () => {
  it("T-PY2MA.Overlay.5: window.__frondoseSetMode('auto') updates mode-badge textContent and className", () => {
    // Given: overlay installed; modeBadge.textContent === "MANUAL"; className includes "manual"
    // When:  window.__frondoseSetMode("auto") is called
    // Then:  modeBadge.textContent === "AUTO"
    //        AND modeBadge.className === "mode-badge auto" (class replaced, not appended)
    const { modeBadge, __frondoseSetMode } = makeOverlayImpl();

    __frondoseSetMode("auto");
...
describe("T-PY2MA.Overlay.6 — __frondoseSetMode('magical') sets badge text to 'MAGICAL' + .magical class (G-PY2MA.7 / NG-10 hook)", () => {
  it("T-PY2MA.Overlay.6: window.__frondoseSetMode('magical') updates mode-badge for defensive Magical support", () => {
    // Given: overlay installed; modeBadge.textContent === "MANUAL"
    // When:  window.__frondoseSetMode("magical") is called
    // Then:  modeBadge.textContent === "MAGICAL"
    //        AND modeBadge.className === "mode-badge magical"
    const { modeBadge, __frondoseSetMode } = makeOverlayImpl();

    __frondoseSetMode("magical");
...
describe("T-PY2MA.Overlay.7 — __frondoseAppendChunk auto-opens agent bubble when none active (G-PY2MA.6 / NEW-BLOCKER-1)", () => {
  it("T-PY2MA.Overlay.7: when activeAgentTextEl===null and __frondoseAppendChunk('Hello ') fires, a .msg-agent bubble is auto-created", () => {
    // Given: overlay installed; activeAgentTextEl === null (cleared by __frondoseEndAgent); convList has 0 children
    //        C-1 migration: __frondoseAppendOutput routes through __frondoseAppendChunk
    // When:  window.__frondoseAppendChunk("Hello ") fires (or: __frondoseAppendOutput("Hello ") → routes to __frondoseAppendChunk)
    ...
    const { convList, __frondoseAppendChunk, getActiveEl } = makeOverlayImpl();

    // activeAgentTextEl starts at null (no __frondoseBeginAgent called)
    ...
    assert.ok(
      msgAgent !== undefined,
      "a .msg-agent bubble must be auto-created by __frondoseAppendChunk when activeAgentTextEl===null (NEW-BLOCKER-1 fix, G6). " +
    ...
    // Also verify via C-1 migration path (__frondoseAppendOutput → __frondoseAppendChunk)
    const { convList: cl2, __frondoseAppendOutput, getActiveEl: getAel2 } = makeOverlayImpl();
    assert.equal(getAel2(), null, "pre-condition: fresh overlay activeAgentTextEl must be null");
    __frondoseAppendOutput("via C-1");
    const msgAgent2 = cl2.children.find((c) => c.classList.contains("msg-agent"));
    assert.ok(
      msgAgent2 !== undefined,
      "__frondoseAppendOutput (C-1 legacy wrapper) must also trigger auto-open via __frondoseAppendChunk (G6 C-1 migration).",
    );
...
describe("T-PY2MA.Overlay.8 — __frondoseEndAgent closes active bubble WITHOUT wiping prior bubbles (G-PY2MA.6 / NEW-BLOCKER-1 + C-2)", () => {
  it("T-PY2MA.Overlay.8: after 2 full turns rendered, __frondoseEndAgent() sets activeAgentTextEl=null; prior bubbles persist", () => {
    ...
    //        C-2 migration: __frondoseClearOutput() routes to __frondoseEndAgent()
    // When:  window.__frondoseEndAgent() fires (or: __frondoseClearOutput() → routes to __frondoseEndAgent())
    ...
    // Turn 1
    __frondoseAppendUser("user1");
    __frondoseBeginAgent();
    __frondoseAppendChunk("reply1");
    __frondoseEndAgent();
    // Turn 2
    __frondoseAppendUser("user2");
    __frondoseBeginAgent();
    __frondoseAppendChunk("reply2");
    ...
    // Test via __frondoseEndAgent (C-2 semantics: close bubble, NOT wipe screen)
    __frondoseEndAgent();

    assert.equal(getActiveEl(), null, "__frondoseEndAgent must set activeAgentTextEl=null (G6 end-of-turn)");
    assert.equal(
      convList.children.length,
      4,
      "convList must still have 4 children after __frondoseEndAgent (G6 NO DOM wipe — C-2 semantics). " +
        "If convList.children.length < 4, __frondoseEndAgent incorrectly wiped prior bubbles.",
    );

    // Also verify via C-2 migration path (__frondoseClearOutput → __frondoseEndAgent)
    const {
      convList: cl2,
      __frondoseBeginAgent: ba2,
      __frondoseAppendChunk: ac2,
      __frondoseClearOutput: co2,
      getActiveEl: gae2,
    } = makeOverlayImpl();
    ba2();
    ac2("some text");
    assert.notEqual(gae2(), null, "pre-condition: activeAgentTextEl set");
    co2(); // C-2: __frondoseClearOutput → __frondoseEndAgent
    assert.equal(gae2(), null, "__frondoseClearOutput (C-2) must set activeAgentTextEl=null (G6 C-2 semantics)");
    assert.equal(
      cl2.children.length,
      1,
      "convList must still have 1 child after __frondoseClearOutput (C-2 NO wipe — only closes active bubble).",
    );
  });
});
...
describe("T-PY2MA.Overlay.Migration — C-1/C-2/C-3 source-structural checks", () => {
  it("T-PY2MA.Overlay.Migration.1 (C-1): bootstrapLegacy.ts __frondoseAppendOutput routes to __frondoseAppendChunk (not dialogElements.output)", () => {
    // Given: src/overlay/bootstrapLegacy.ts source
    // When:  scanned for __frondoseAppendChunk call inside __frondoseAppendOutput body
    // Then:  the routing call is present AND 'dialogElements.output.textContent' direct write is ABSENT
    const hasChunkCall = LEGACY_TS.includes("window.__frondoseAppendChunk");
    const hasOldDirectWrite = LEGACY_TS.includes("dialogElements.output.textContent");
    assert.ok(
      hasChunkCall,
      "bootstrapLegacy.ts __frondoseAppendOutput must route to window.__frondoseAppendChunk (C-1 migration). " +
  ...
  it("T-PY2MA.Overlay.Migration.2 (C-2): bootstrapLegacy.ts __frondoseClearOutput routes to __frondoseEndAgent (semantics change)", () => {
    // Given: src/overlay/bootstrapLegacy.ts source
    // When:  scanned for __frondoseEndAgent call inside __frondoseClearOutput body
    // Then:  the routing call is present; the 'semantics changed' comment marker is present
    const hasEndAgentCall = LEGACY_TS.includes("window.__frondoseEndAgent()");
    const hasSemanticsComment = LEGACY_TS.includes("semantics changed");
    assert.ok(
      hasEndAgentCall,
      "bootstrapLegacy.ts __frondoseClearOutput must call window.__frondoseEndAgent() (C-2 migration). " +
        "Without this, clearing output between turns keeps activeAgentTextEl pointing at a stale element.",
    );
    assert.ok(
      hasSemanticsComment,
      "bootstrapLegacy.ts __frondoseClearOutput must have 'semantics changed' comment marker (C-2 — no DOM wipe). " +
        "The marker confirms the intentional behavior change from 'wipe textContent' to 'close bubble'.",
    );
  });

  it("T-PY2MA.Overlay.Migration.3 (C-3): bootstrap.ts boot-replay uses __frondoseBeginAgent + __frondoseAppendChunk + __frondoseEndAgent", () => {
    // Given: src/overlay/bootstrap.ts source
    // When:  scanned for __frondoseBeginAgent + __frondoseAppendChunk + __frondoseEndAgent calls in the boot-replay block
    // Then:  all three are present AND the old 'dialogElements.output.textContent = frondoseDialogState.output'
    //        direct write is ABSENT from the boot-replay block
    const hasBeginAgent = BOOTSTRAP_TS.includes("window.__frondoseBeginAgent()");
    const hasAppendChunk = BOOTSTRAP_TS.includes("window.__frondoseAppendChunk(frondoseDialogState.output)");
    const hasEndAgent = BOOTSTRAP_TS.includes("window.__frondoseEndAgent()");
    const hasOldDirectWrite = BOOTSTRAP_TS.includes("dialogElements.output.textContent = frondoseDialogState.output");
    assert.ok(
      hasBeginAgent && hasAppendChunk && hasEndAgent,
      "bootstrap.ts boot-replay block must use window.__frondoseBeginAgent() + __frondoseAppendChunk(frondoseDialogState.output) + __frondoseEndAgent() (C-3 migration). " +
        `Found: __frondoseBeginAgent=${hasBeginAgent}, __frondoseAppendChunk=${hasAppendChunk}, __frondoseEndAgent=${hasEndAgent}.`,
    );
    assert.equal(
      hasOldDirectWrite,
      false,
      "bootstrap.ts boot-replay must NOT write dialogElements.output.textContent = frondoseDialogState.output directly (C-3 — old pattern removed).",
    );
  });
});
...
describe("T-PY2MA.Overlay.SRC — Sketch C: new __mai* helpers defined in bootstrapShell.ts", () => {
  it("T-PY2MA.Overlay.SRC.1: bootstrapShell.ts defines window.__frondoseAppendUser, __frondoseBeginAgent, __frondoseAppendChunk, __frondoseEndAgent", () => {
    // Given: src/overlay/bootstrapShell.ts source (pre-builder: only has __frondoseSetMode)
    ...
    const hasAppendUser = SHELL_TS.includes("window.__frondoseAppendUser");
    const hasBeginAgent = SHELL_TS.includes("window.__frondoseBeginAgent");
    const hasAppendChunk = SHELL_TS.includes("window.__frondoseAppendChunk");
    const hasEndAgent = SHELL_TS.includes("window.__frondoseEndAgent");
    assert.ok(
      hasAppendUser && hasBeginAgent && hasAppendChunk && hasEndAgent,
      `bootstrapShell.ts must define all 4 window.__mai* helpers. ` +
        `Found: __frondoseAppendUser=${hasAppendUser}, __frondoseBeginAgent=${hasBeginAgent}, ` +
        `__frondoseAppendChunk=${hasAppendChunk}, __frondoseEndAgent=${hasEndAgent}. ` +
        `FAILS pre-builder (Sketch C §5.3.5 not yet pasted).`,
    );
  });

  it("T-PY2MA.Overlay.SRC.2: bootstrapShell.ts __frondoseAppendChunk has the auto-open guard (activeAgentTextEl === null branch)", () => {
    ...
    // Then:  the guard code (typeof window.__frondoseBeginAgent === 'function') is present
    const hasAutoOpenGuard = SHELL_TS.includes("typeof window.__frondoseBeginAgent === 'function'");
    assert.ok(
      hasAutoOpenGuard,
      "bootstrapShell.ts __frondoseAppendChunk must have the auto-open guard " +
        "'typeof window.__frondoseBeginAgent === 'function'' (NEW-BLOCKER-1 fix). " +
