...
describe("B3: client.scroll() uses window.scrollBy (evaluate-based, no synthesizeScrollGesture)", () => {
  it(
    "T-B3.1: when CdpClient.scroll('down', 3500), evaluate receives 'window.scrollBy(0, 3500)' and synthesizeScrollGesture/getLayoutMetrics are NOT called",
    async () => {
      ...
      assert.equal(getLayoutMetricsCalled, false, "Page.getLayoutMetrics must NOT be called (no viewport coordinate needed)");
      assert.equal(synthesizeScrollGestureCalled, false, "Input.synthesizeScrollGesture must NOT be called");
    },
  );

  it(
    "T-B3.2: scroll direction-to-delta mapping — up→(0,-N), left→(-N,0), right→(N,0) — no synthesizeScrollGesture in any direction",
    async () => {
