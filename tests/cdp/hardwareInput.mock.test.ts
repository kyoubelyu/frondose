...
function makeFakeCg(): { cg: CgEvent; log: FakeCgEventLog } {
  ...
  const cg: CgEvent = {
    moveMouse(x, y) {
      log.moveMouse.push([x, y]);
    },
    mouseClick() {
      log.mouseClick++;
    },
    scrollWheel(dx, dy) {
      log.scrollWheel.push([dx, dy]);
    },
    keyEvent(keyCode, down, flags) {
      log.keyEvent.push([keyCode, down, flags]);
    },
    unicodeType(text) {
      log.unicodeType.push(text);
    },
    getMousePos() {
      log.getMousePosCalls++;
      return { x: 0, y: 0 };
    },
    getScreenSize() {
...
describe("resolveInputMode — G-P32.2 / G-P32.3 / G-P32.4 / G-P32.5", () => {
  it("T-RIM.1: when requested mode is 'cdp', resolveInputMode returns 'cdp' without calling loadCg", () => {
  ...
  it("T-RIM.2: when requested is 'hardware' and loadCg throws, resolveInputMode returns 'cdp' + stderr warning", () => {
  ...
  it("T-RIM.3: when loadCg succeeds but isAccessibilityTrusted() is false, returns 'cdp' + stderr warning", () => {
  ...
  it("T-RIM.4: when loadCg succeeds and isAccessibilityTrusted() is true, returns 'hardware'", () => {
...
describe("hardwareClickAt — G-P32.15", () => {
  it("T-HW.CLICK.1: hardwareClickAt resolves coords, calls moveMouse ≥1× along a curve, calls mouseClick exactly once", async () => {
...
describe("hardwareTypeAt — G-P32.16 (incl. CONCERN-1 UTF-16 guard)", () => {
  it("T-HW.TYPE.1: hardwareTypeAt focuses element (moveMouse+mouseClick), then Cmd+A, then unicodeType(text)", async () => {
  ...
  it("T-HW.TYPE.2 [CONCERN-1]: hardwareTypeAt passes non-ASCII text (é, café) verbatim to unicodeType", async () => {
...
describe("hardwareScroll — G-P32.17", () => {
  it("T-HW.SCROLL.1: hardwareScroll(down, 3500) calls scrollWheel multiple times with negative dy; 'up' gives positive dy", async () => {
    ...
    assert.ok(
      logUp.scrollWheel.every(([, dy]) => dy > 0),
...
describe("hardwarePressKey — G-P32.18 / G-P32.14", () => {
  it("T-HW.PRESS.1: hardwarePressKey('Enter', 0, fakeCg) posts keyEvent(36,true,0) then keyEvent(36,false,0)", () => {
  ...
  it("T-HW.PRESS.2: hardwarePressKey with unknown key 'NoSuchKey' throws a clear error (G-P32.14)", () => {
