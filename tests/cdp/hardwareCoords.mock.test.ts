...
describe("resolveScreenCoords — G-P32.11", () => {
  it(
    "T-COORD.1: resolveScreenCoords returns {x: sx+(b0+b4)/2, y: sy+ch+(b1+b5)/2} (Method A arithmetic)",
    async () => {
  ...
  it(
    "T-COORD.2: resolveScreenCoords with a ref absent from currentRefMap throws a clear Error",
    async () => {
...
describe("mouseCurve — G-P32.12", () => {
  it(
    "T-CURVE.1: mouseCurve({x:0,y:0},{x:300,y:200}) returns ≥1 points; last===to; steps bounded; trends toward target",
    () => {
  ...
  it(
    "T-CURVE.2: mouseCurve(p, p) returns [p] (already at target — no empty array, no teleport)",
    () => {
...
describe("jitter — G-P32.13", () => {
  it(
    "T-JIT.1: jitter(100, 4) over 1000 calls always returns a value within [96, 104]",
    () => {
      // Given: v=100, max=4
      ...
      for (const r of results) {
        assert.ok(
          r >= 96 && r <= 104,
          `T-JIT.1: jitter(100, 4) returned ${r}, outside [96, 104]`,
        );
      }
    },
  );
...
describe("mapKey / mapModifiers — G-P32.14", () => {
  it(
    "T-KEY.1: mapKey table — Enter→36, Tab→48, Escape→53, ArrowDown→125, Space→49; modifier bitmask→CGEventFlags",
    () => {
