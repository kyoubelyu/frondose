 ...
 *   T-Sidecar.Main.1  — main() registers crash handlers + calls runAppBackend
...
describe("T-Sidecar.Main — main() integration and ESM guard", () => {
  it("T-Sidecar.Main.1: when called with valid argv, main() invokes registerCrashHandlers then runAppBackend then process.exit(0)", async () => {
    // Given: stub runAppBackend resolves immediately; registerCrashHandlers is a spy;
    //        valid argv contains --sock and --token
    // When:  main() runs to completion
    // Then:  registerCrashHandlers called once BEFORE runAppBackend;
    //        runAppBackend called with { sockPath, bearerToken };
    //        process.exit(0) called after await resolves
    assert.ok(sourceExists, "src/app/sidecarMain.ts must exist (pre-impl: intentional scaffold failure)");

    const crashLoggerUrl = pathToFileURL(resolve(process.cwd(), "src/app/crashLogger.js")).href;
    const serveUrl = pathToFileURL(resolve(process.cwd(), "src/app/backend.js")).href;
    ...
    mock.module(serveUrl, {
      namedExports: {
        runAppBackend: async (opts: unknown) => {
          capturedServeOpts = opts;
          callOrder.push("runAppBackend");
        },
      },
    });
    ...
    try {
      const mod = (await import(`${pathToFileURL(SIDECAR_SRC).href}?bust=${Date.now()}`).catch(
        () => null,
      )) as SidecarModule | null;
    ...
    // crashHandlers BEFORE runAppBackend
    const crashIdx = callOrder.indexOf("registerCrashHandlers");
    const serveIdx = callOrder.indexOf("runAppBackend");
    assert.ok(crashIdx !== -1, "registerCrashHandlers must be called");
    assert.ok(serveIdx !== -1, "runAppBackend must be called");
    assert.ok(crashIdx < serveIdx, "registerCrashHandlers must be called BEFORE runAppBackend");
    assert.deepEqual(capturedServeOpts, { portFile: "/tmp/main1.port", bearerToken: "tok1" });
    assert.ok(
      exitCalls.includes(0),
      `process.exit(0) must be called after serve resolves; got: ${JSON.stringify(exitCalls)}`,
    );
  });
  ...
  it("T-Sidecar.Main.2: importing the module with a test-runner argv[1] does NOT auto-invoke main()", async () => {
    ...
    //        runAppBackend not called; process.exit not called; no socket opened
    assert.ok(sourceExists, "src/app/sidecarMain.ts must exist (pre-impl: intentional scaffold failure)");
    ...
    assert.equal(serveCallCount.n, 0, "runAppBackend must NOT be called at import time");
  ...
  it("T-Sidecar.Main.3: crash-handler is registered BEFORE the serve graph dynamic-import fires", async () => {
    ...
    // runAppBackend tries to call, which causes the dynamic import to
    ...
    mock.module(serveUrl, {
      namedExports: {
        runAppBackend: async () => {
          callOrder.push("runAppBackend_throw");
    ...
    try {
      const mod = (await import(`${pathToFileURL(SIDECAR_SRC).href}?bust3=${Date.now()}`).catch(
        () => null,
      )) as SidecarModule | null;
    ...
    const crashIdx = callOrder.indexOf("registerCrashHandlers");
    const serveThrowIdx = callOrder.indexOf("runAppBackend_throw");
    assert.ok(crashIdx !== -1, "registerCrashHandlers must be called");
