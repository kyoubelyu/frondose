    ...
    await runAgentLoopPi({
      ...
      onStepFinish: async (step: StepResult<ToolSet>) => {
        ...
        for (const tr of toolResults) {
          ...
          if (tr.toolName === "end_auto_run" && args.isCronTurn !== true) {
            ...
            const env = tr.result as
              | { ok?: boolean; data?: { runId?: string; alreadyEnded?: boolean } }
              | null
              | undefined;
            const runId = env?.data?.runId;
            if (env?.ok === true && env.data?.alreadyEnded !== true && typeof runId === "string") {
              const db = getSalesDb(deps.salesDbPath);
