...
export function createAppActions(deps) {
    ...
    async function applyLanguagePref() {
        try {
            ...
            if (nextLocale !== getLocale()) {
                ...
                localizeDocument(deps.document, { force: true });
            }
        }
        catch (e) {
    ...
    async function loadIdentity() {
        try {
            const r = await deps.invoke("frondose_identity");
            if (r.ok === false) {
    ...
    async function performRetry() {
        ...
        try {
            const r = await deps.invoke("frondose_agent_retry");
            if (r.ok === false) {
    ...
    async function approveWorkflowStep() {
        ...
        if (workflowView?.pendingStepId === null || workflowView === null)
            return;
        try {
    ...
    async function declineWorkflowStep() {
        const workflowView = deps.getWorkflowView();
        if (workflowView?.pendingStepId === null || workflowView === null)
            return;
        try {
