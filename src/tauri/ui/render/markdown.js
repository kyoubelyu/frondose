...
function normalizeUrl(raw) {
    ...
    for (const ch of raw) {
        ...
        if (code <= 0x20 || code === 0x7f)
            continue; // drop C0 controls, space, and DEL
        out += ch;
    }
    return out;
}
...
function safeHref(raw) {
    ...
    if (match === null)
        return null;
    if (!SAFE_URL_SCHEMES.has((match[1] ?? "").toLowerCase()))
        return null;
    return normalized;
}
...
function buildLink(doc, label, rawUrl) {
...
function buildInlineCode(doc, text) {
...
function parseInline(doc, text) {
    ...
    for (; m !== null; m = INLINE_RE.exec(text)) {
        if (m.index > lastIndex)
            nodes.push(span(doc, "", text.slice(lastIndex, m.index)));
        if (m[1] !== undefined)
            nodes.push(buildInlineCode(doc, m[1]));
        else if (m[2] !== undefined)
            nodes.push(span(doc, "md-bold", m[2]));
        else if (m[3] !== undefined)
            nodes.push(span(doc, "md-italic", m[3]));
        else if (m[4] !== undefined && m[5] !== undefined)
            nodes.push(buildLink(doc, m[4], m[5]));
        else if (m[6] !== undefined)
            nodes.push(buildLink(doc, m[6], m[6]));
        lastIndex = INLINE_RE.lastIndex;
    }
    if (lastIndex < text.length)
        nodes.push(span(doc, "", text.slice(lastIndex)));
    if (nodes.length === 0)
        nodes.push(span(doc, "", text));
    return nodes;
}
...
function buildParagraph(doc, text) {
    const p = el(doc, "p", "md-p");
    for (const node of parseInline(doc, text))
        p.appendChild(node);
    return p;
}
function buildHeading(doc, level, text) {
...
function buildCodeBlock(doc, text) {
...
function buildList(doc, ordered, items) {
    const list = el(doc, ordered ? "ol" : "ul", "md-list");
    for (const item of items) {
        const li = el(doc, "li");
        for (const node of parseInline(doc, item))
            li.appendChild(node);
        list.appendChild(li);
    }
    return list;
}
function buildTable(doc, header, rows) {
    const table = el(doc, "table", "md-table");
    const thead = el(doc, "thead");
    const headRow = el(doc, "tr");
    for (const cell of header) {
        const th = el(doc, "th");
        for (const node of parseInline(doc, cell))
            th.appendChild(node);
...
export function buildMarkdownNodes(doc, raw) {
    const nodes = [];
...
export function renderMarkdownInto(doc, container, raw) {
    container.textContent = ""; // clears existing children (same pattern as render/dom.ts:clear)
    for (const node of buildMarkdownNodes(doc, raw))
