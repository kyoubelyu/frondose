...
// com.kyoube.mai.update-server launchd service from the public CLI command
...
// What we DO need:
//   - registerCrashHandlers() FIRST (static import), installed BEFORE the
//     update-server graph is dynamic-imported, so a RUNTIME uncaught exception /
//     rejection after boot hits the registered handlers (~/.mai/agent/logs/crash.log).
...
//     supplies port 4875 + ~/.mai/site defaults (§1.4) — exactly what the live
