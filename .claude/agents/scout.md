...
- **Step 1 — Research.** Produce `docs/phase-N-research.md`. Verify upstream / external behavior against authoritative sources (Vercel AI SDK source under `node_modules/ai/**` and `node_modules/@ai-sdk/**`, `chrome-remote-interface` source/docs, `chrome-launcher` docs, `better-sqlite3` docs, mai-linkedin source for ports under `/Users/kyoubelyu/Kyoube-Skills/mai-linkedin/src/**`, Chrome DevTools Protocol docs at chromedevtools.github.io). Cite file:line for every load-bearing claim. Distinguish what you verified from what you assumed.
